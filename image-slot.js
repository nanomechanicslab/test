...
(() => {
  ...
  const ACCEPT = ['image/png', 'image/jpeg', 'image/webp', 'image/avif', 'image/gif', 'image/bmp', 'image/heic', 'image/heif', '.heic', '.heif'];
  ...
  function loadHeic() {
    ...
    heicLib = new Promise((res, rej) => {
      ...
      s.onerror = () => { heicLib = null; rej(new Error('HEIC converter failed to load')); };
  ...
  async function normalizeFile(f) {
    ...
    try { await createImageBitmap(f); return f; } catch (e) {}
    const conv = await loadHeic();
    const out = await conv({ blob: f, toType: 'image/jpeg', quality: 0.9 });
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      this._setError(null);
      if (!file || (ACCEPT.indexOf(file.type) < 0 && !isHeic(file))) {
        this._setError('Drop a PNG, JPEG, WebP, AVIF, GIF, BMP or HEIC image.');
        return;
      }
